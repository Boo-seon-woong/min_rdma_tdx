...
typedef struct {
    ...
    peer_info_t local;
    int shmem_fd;
    struct shm_req shmem_req;
    int shmem_active;
} rdma_t;

static rdma_t *global_rdma;
...
static int parse_config(const char *path, config_t *cfg) {
    ...
    cfg->iterations = 1;
    cfg->queue_depth = 1;
    snprintf(cfg->tdx_shmem_path, sizeof(cfg->tdx_shmem_path), "/dev/tdx_shmem");
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        *eq = '\0';
        ...
        if (strcmp(key, "mode") == 0) {
            ...
            if (parse_u64(value, &u64) != 0 || u64 < sizeof(msg_header_t) || u64 > SIZE_MAX) {
        ...
        } else if (strcmp(key, "tdx_shared") == 0) {
            if (parse_bool(value, &cfg->tdx_shared) != 0) {
                fprintf(stderr, "config:%u invalid tdx_shared\n", line_no);
                fclose(fp);
                return -1;
            }
        } else if (strcmp(key, "tdx_shmem_path") == 0) {
            snprintf(cfg->tdx_shmem_path, sizeof(cfg->tdx_shmem_path), "%s", value);
        } else {
...
static int convert_shared(rdma_t *rdma, const config_t *cfg) {
    if (!cfg->tdx_shared) {
        return 0;
    }

    rdma->shmem_fd = open(cfg->tdx_shmem_path, O_RDWR);
    if (rdma->shmem_fd < 0) {
        perror("open tdx_shmem");
        return -1;
    }
...
static void revert_shared(rdma_t *rdma) {
    if (!rdma->shmem_active) {
        if (rdma->shmem_fd >= 0) {
            close(rdma->shmem_fd);
            rdma->shmem_fd = -1;
        }
    ...
    if (ioctl(rdma->shmem_fd, IOCTL_REVERT, &rdma->shmem_req) < 0) {
        perror("ioctl revert");
    }
    close(rdma->shmem_fd);
    rdma->shmem_fd = -1;
...
int main(int argc, char **argv) {
    ...
    printf("startup mode=%s workload=%s control=%s:%u ib_device=%s ib_port=%u message_size=%zu iterations=%" PRIu64 " queue_depth=%u tdx_shared=%s\n",
        mode_name(cfg.mode),
        cfg.workload == WORKLOAD_CHECK ? "check" : "bench",
        cfg.control_host,
        cfg.control_port,
    ...
    if (remote.region_bytes < (uint64_t)rdma.slot_size * (uint64_t)rdma.depth) {
