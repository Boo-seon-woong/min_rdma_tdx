...
(아래 세팅은 simba의 ~/2026/tdx/guest-tools/run_td.py를 아래 명령어로 실행한 vm의 설정을 안내합니다.)

```jsx
sudo /home/seonung/2026/tdx/guest-tools/run_td --rdma-nics 0000:6f:00.0 --rdma-iommufd off --sm-mode external --tcp-hostfwd-ports 7301 --tcp-hostfwd-bind-addr 10.20.18.199 2>&1
```

- ibstat result
    
    ```jsx
    root@tdx-guest:~# ibstat
    CA 'ibp1s0'
            CA type: MT4129
            Number of ports: 1
            Firmware version: 28.34.1002
...
- rdma link
    ...
    ```jsx
    root@tdx-guest:~# rdma link
    link ibp1s0/1 subnet_prefix fe80:0000:0000:0000 lid 1 sm_lid 2 lmc 0 state ACTIVE physical_state LINK_UP 
    ```
